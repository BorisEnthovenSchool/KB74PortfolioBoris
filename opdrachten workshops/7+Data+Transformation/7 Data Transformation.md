...
```python
df_cp = df.copy()
boxcox = stats.boxcox(df_cp.LotArea)
df_cp.LotArea = -df_cp.LotArea ** boxcox[1]
# df_cp.SalePrice = np.log(df_cp.SalePrice)

sns.distplot(df_cp.LotArea, fit=norm)
plt.show()
stats.probplot(df_cp.LotArea, plot=plt)
plt.show()
data = pd.concat([df_cp.SalePrice, df_cp.LotArea], axis=1)
