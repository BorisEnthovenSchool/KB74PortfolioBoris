...
print(rating.take(2))
first=rating.first()
rating = rating.filter(lambda row: row != first)
rating.take(5)
```

    ['userID,placeID,rating,food_rating,service_rating', 'U1077,135085,2,2,2']
...
```python
rawRDD = sc.textFile("userprofile.csv")
print(rawRDD.take(2))
splitRawRDD = rawRDD.map(lambda row: row.split(','))
userbudgetWithHeader = splitRawRDD.map(lambda x: (x[0], x[17]))
userFirstRow = userbudgetWithHeader.first()
print(userFirstRow)
userbudget = userbudgetWithHeader.filter(lambda row: row != userFirstRow)
userbudget.take(5)
```

    ['userID,latitude,longitude,smoker,drink_level,dress_preference,ambience,transport,marital_status,hijos,birth_year,interest,personality,religion,activity,color,weight,budget,height', 'U1001,22.139997,-100.978803,false,abstemious,informal,family,on foot,single,independent,1989,variety,thrifty-protector,none,student,black,69,medium,1.77']
    ('userID', 'budget')
...
```python
splitrating = rating.map(lambda row: row.split(','))
userrating = splitrating.map(lambda x: (x[0], x[2]))
userrating.take(5)
```
...
```python
joinedratingsbudget = userbudget.join(userrating).map(lambda row: (row[1][0], int(row[1][1])))
joinedratingsbudget.take(5)
```
...
```python
averagePerKey = joinedratingsbudget.groupByKey().mapValues(lambda l: sum(l)/len(l))
averagePerKey.take(5)
```
...
```python
userRawRDD = sc.textFile('userprofile.csv')
print(userRawRDD.first())
userpos = userRawRDD.map(lambda row: row.split(','))\
    .map(lambda element: (element[0],(element[1], element[2])))
header = userpos.first()
userpos = userpos.filter(lambda row: row != header)
userpos.take(5)
```
...
```python
rawPlaceRDD = sc.textFile("geoplaces2.csv")
print(rawPlaceRDD.first())
placeRDD = rawPlaceRDD.map(lambda row: row.split(","))\
    .map(lambda element: (element[0], (element[1], element[2])))
headerPlace = placeRDD.first()
placepos = placeRDD.filter(lambda row: row != headerPlace)
placepos.take(5)
```
...
```python
placeDict = placepos.collectAsMap()
placeDict
```
...
```python
# placepos.take(5) --> (PlaceID, (lat, lon))
# userpos.take(5)  --> (UserID,  (lat, lon))
# userrating       --> (UserID, rating)
# solution=        --> (rating,  vincenti)

rating = sc.textFile("rating_final.csv")
newUserRating = rating.map(lambda row: row.split(',')).map(lambda x: (x[0], x[1], x[2]))
print(newUserRating.take(4))

rawPlaceRDD = sc.textFile("geoplaces2.csv")
newPlacePos = rawPlaceRDD.map(lambda row: row.split(",")).map(lambda element: (element[0], (element[1], element[2])))
print(newPlacePos.take(3))
    
userRawRDD = sc.textFile('userprofile.csv')
newUserPos = userRawRDD.map(lambda row: row.split(',')).map(lambda element: (element[0],(element[1], element[2])))
print(newUserPos.take(4))
...
```
    ...
    [('userID', 'placeID', 'rating'), ('U1077', '135085', '2'), ('U1077', '135038', '2'), ('U1077', '132825', '2')]
    [('placeID', ('latitude', 'longitude')), ('134999', ('18.915421', '-99.184871')), ('132825', ('22.1473922', '-100.983092'))]
    [('userID', ('latitude', 'longitude')), ('U1001', ('22.139997', '-100.978803')), ('U1002', ('22.150087', '-100.983325')), ('U1003', ('22.119847', '-100.946527'))]
